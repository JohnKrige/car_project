...
def scrape_data(popular_brands=True):
	ua = UserAgent()
	user_agent = ua.ff 
	...
	for car_brand in brand_urls:
		...
		page_request = requests.get(page_url,headers=req_headers)
		...
		for page in range(1,total_pages+1):
			...
			try: 
				...
				request = requests.get(url_car,headers=req_headers)
				...
				for car in cars_on_page:
					...
					try: 
						...
						print("Kicked from specific url, sleep for {} seconds".format(sleep_time))
						time.sleep(sleep_time)


			except:
				print("Kicked from brand page, sleep for {} seconds".format(sleep_time))
				time.sleep(sleep_time)		
